...
try {
  rawInputData = fs.readFileSync(`${__dirname}/D11/input.txt`, 'utf8')
} catch (e) {
  console.log(`Error!`)
  console.error(e)
}
...
const getNeighborCoords = (xCoord, yCoord, gridHeight, gridWidth) => {
  let neighborCoords = []

  // get neighbors' coordinates clockwise
  if (!!xCoord)
    neighborCoords.push([xCoord - 1, yCoord])

  if (!!xCoord && yCoord + 1 < gridWidth)
    neighborCoords.push([xCoord - 1, yCoord + 1])

  if (yCoord + 1 < gridWidth)
    neighborCoords.push([xCoord, yCoord + 1])

  if (xCoord + 1 < gridHeight && yCoord + 1 < gridWidth)
    neighborCoords.push([xCoord + 1, yCoord + 1])

  if (xCoord + 1 < gridHeight)
    neighborCoords.push([xCoord + 1, yCoord])

  if (xCoord + 1 < gridHeight && !!yCoord)
    neighborCoords.push([xCoord + 1, yCoord - 1])
    ...
    do {
      Array
        ...
        .forEach( ({ position, xCoord, yCoord, energyLevel, hasFlashed }) => {
          ...
          getNeighborCoords( xCoord, yCoord, energyLevel.length, columnCount)
            .filter(  ([ nCoordX, nCoordY ]) => ++energyLevels[nCoordX][nCoordY] > MAX_ENERGY_LEVEL )
            .forEach( ([ nCoordX, nCoordY ]) => {
